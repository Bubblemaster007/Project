...
def _load_existing_gan_paths(spec: DatasetSpec, out_dir: Path, cfg: ModelPoolConfig) -> tuple[Path | None, Path | None]:
    val_target = out_dir / f"generated_val_{GAN_METHOD}.npy"
    test_target = out_dir / f"generated_samples_{GAN_METHOD}.npy"
    candidate_roots = [
        BASE_DIR / "results" / "gan_augmented_tailweighted_copula_valprotected_formal50" / spec.out_name,
        BASE_DIR / "results" / "gan_augmented_tailweighted_copula" / spec.out_name,
    ]
    for gan_root in candidate_roots:
    ...
    try:
        val_source, test_source = ensure_gan_augmented_val_test_paths(spec, gan_cfg, gan_cfg.out_dir / spec.out_name)
    except Exception:
        return None, None
    if val_source.exists() and test_source.exists():
        shutil.copy2(val_source, val_target)
