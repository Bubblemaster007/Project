...
def _generated_path_for_split(out_dir: Path, split: str) -> Path:
    return out_dir / ("generated_samples_gan_aug_tailweighted.npy" if split == "test" else f"generated_{split}_gan_aug_tailweighted.npy")
...
def _run_augmented_tailweighted(
...
) -> tuple[dict[str, Path], pd.DataFrame, pd.DataFrame, str]:
    ...
    kept_x = np.empty((0, 3, int(_to_channel_time(x_train).shape[2])), dtype=np.float32)
    kept_meta = train_meta_full.iloc[:0].copy()
    kept_mask = mask_train[:0] if mask_train is not None else None
    n_gan_candidates = 0
    n_after_physics = 0
    n_after_risk = 0
    corr_before = np.nan
    corr_after = np.nan
    filter_note = threshold_note
    ...
    if len(tail_idx) < int(cfg.min_tail_samples_after_relax):
        ...
        x_phys, meta_phys, mask_phys, _ = physical_filter_and_clip(x_gan_candidates, meta_gan_candidates, gan_cfg, mask_gan_candidates)
        ...
        n_after_physics = int(len(x_phys))
        n_after_risk = int(len(x_risk))
        n_keep = min(max(int(round(float(cfg.gan_keep_ratio) * len(x_train))), int(cfg.gan_keep_min)), len(tail_idx))
        kept_x, kept_meta, kept_mask, _, corr_summary = _correlation_filter_candidates(
            x_risk,
            meta_risk,
            mask_risk,
            risk_candidates,
        ...
        corr_before = corr_summary["corr_error_before"]
        corr_after = corr_summary["corr_error_after"]
        filter_note = f"{risk_note}; {corr_summary['corr_filter_note']}"
        if len(kept_x) == 0:
            gan_note = f"GAN trained but no samples survived filters. {filter_note}"
        else:
    ...
    if len(kept_x):
        ...
        x_train_aug = _to_channel_time(x_train).astype(np.float32)
        cond_train_aug_raw = cond_train_raw.reset_index(drop=True).copy()
        meta_train_aug = meta_train.reset_index(drop=True).copy()
        mask_train_aug = mask_train
        ...
        [
            {
                ...
                "n_after_physics_filter": int(n_after_physics),
                "n_after_risk_filter": int(n_after_risk),
                "n_after_corr_filter": int(len(kept_x)),
                "n_gan_kept": int(len(kept_x)),
                "corr_error_before": corr_before,
                "corr_error_after": corr_after,
                "notes": filter_note,
            }
        ]
    )
    ...
    for split in splits:
        ...
        split_candidates, split_metrics, split_targets, split_base = _generate_candidate_pool(
            split, cond_split, mask_split, models_aug, train_risk_aug, tau_by_month_aug, cfg, cop_cfg, rng, out_dir, spec.out_name
        )
        target_path = out_dir / ("risk_target_summary.csv" if split == "test" else f"risk_target_summary_{split}.csv")
        pd.DataFrame(split_targets).to_csv(target_path, index=False, encoding="utf-8-sig")
        generated, selection_log = _select_candidates(
            split_candidates, split_metrics, split_targets, split_base, train_risk_aug, cond_split, fixed_weights, cfg
        )
        log_path = out_dir / ("candidate_selection_log_fixed.csv" if split == "test" else f"candidate_selection_log_fixed_{split}.csv")
        selection_log.to_csv(log_path, index=False, encoding="utf-8-sig")
        generated_path = _generated_path_for_split(out_dir, split)
        np.save(generated_path, generated.astype(np.float32))
        generated_paths[split] = generated_path
    return generated_paths, tail_summary, filter_summary, gan_note
...
def run_one_dataset(spec: DatasetSpec, cfg: GanAugmentedTailWeightedConfig) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    generated_paths, tail_summary, filter_summary, gan_note = _run_augmented_tailweighted(spec, cfg, out_dir, splits=("test",))
    generated_final = generated_paths["test"]
...
def ensure_gan_augmented_val_test_paths(
    spec: DatasetSpec,
    cfg: GanAugmentedTailWeightedConfig,
    out_dir: Path,
) -> tuple[Path, Path]:
    out_dir.mkdir(parents=True, exist_ok=True)
    val_path = _generated_path_for_split(out_dir, "val")
    test_path = _generated_path_for_split(out_dir, "test")
    if val_path.exists() and test_path.exists():
        return val_path, test_path
    generated_paths, _, _, _ = _run_augmented_tailweighted(spec, cfg, out_dir, splits=("val", "test"))
    return generated_paths["val"], generated_paths["test"]
