...
- 本机已找到外部论文冻结候选库，并于 2026-09-24 重新运行历史日历、局部 LP 认证和全局分配：72 个请求、69 个嵌入、3 个失败。本次使用已有候选库（regenerate=false），不表示重新训练或生成；失败请求继续保留。跨机器仍须自行配置完整论文依赖。
- 条件事件样本现在同时对设备故障/修复和源荷进行联合抽样：保留第 0 个参考轨迹，其余样本采用配置中的 AR(1) 时间相关负荷及风光残差，并在事件级阶段指标与配对敏感性分析中使用同一组样本。这是未校准的附加敏感性模型，扰动轨迹未经论文 LP 重新认证，不能将其区间解释为论文模型的统计置信度。
- 在缺少本地故障统计时，已加入 `data/fragility/baseline_fragility.csv` 作为文献工程先验，提供 base、conservative、optimistic 三种情景；其曲线和小时强度换算由 `src/chapter3/fragility_curves.py` 统一实现，并明确标记为低可信度、待阿拉山口数据校准。
...
使用 `/Users/bubble/Code_Paper/.venv/bin/python` 安装缺少的 pytest、openpyxl 后，项目测试 35 项通过，独立分阶段测试 1 项通过。阿拉山口实测参数暂不纳入本阶段验收；设备故障率和修复参数仍为联调假设。


### 论文候选库到分阶段平衡的完整联调验收

2026-09-24 在本机使用已有论文冻结候选库（非重新训练/生成），完成历史日历 → 局部 LP 认证 → 全局嵌入 → 连续线路故障修复 → IEEE 33 节点逐时调度 → 四阶段条件样本分析。运行命令：

```bash
python run_full_pipeline.py --config configs/route_acceptance_case33.json
```

配置中的论文根目录按实际机器设置，或使用 CODE_PAPER_ROOT。本机使用 Code_Paper/.venv 的 Python。

输出在 `outputs/route_acceptance_20260924/current_paper/singleton/2021/seed42/`：8760 小时、72 个请求中 69 个成功嵌入、69 个事件各 10 个样本、276 行阶段指标。年度功率账本最大残差 7.47e-12 kW，交流电压/上级容量检查通过。单条年度轨迹的失负荷时间比例为 4.7717%，失负荷 418 小时、缺供电量 128095.12 kWh。详细验收记录见同目录 `route_verification.json`（手工验收产物，主入口不自动生成）。

这完成当前“先跑通技术路线”的联调范围，不表示全部请求嵌入、统计收敛或阿拉山口风险已校准。事件采用 6/6/12/12 小时测试阶段；条件样本固定事件初始 SOC，不能当作独立全年蒙特卡洛样本。AR(1) 为附加敏感性假设，正式概率研究应复用并校准论文多场景分布。阿拉山口真实参数按当前要求暂缓。
